...
function createNewElement(task, finished) {
    ...
    if(finished){
        ...
        checkbox.className = 'checkbox';
        checkbox.innerHTML = '<i class="fa fa-square-o"></i>';
    }
    ...
    .then(function (response) {
        response.json().then(function (data) {
            ...
            let out1 = '';
                for (let i in arr) {
                    out1 += i + ' --- ' + arr[i] + '<br>';
                    document.querySelector('#modal').innerHTML = out1;
                    }
        });
